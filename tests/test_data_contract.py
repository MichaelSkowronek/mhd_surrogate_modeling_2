...
(built by scripts/convert_to_zarr.py) -- they're skipped automatically
otherwise, including in CI, since the ~9GB store isn't checked into git.

The zarr store path and dataset list come from configs/split.yaml rather
...
SPLIT_CONFIG = Path("configs/split.yaml")
CHUNK_T = 64
...
pytestmark = pytest.mark.skipif(
    ...
    reason="zarr store not present locally (build it with scripts/convert_to_zarr.py)",
)
